...
import { GetAllMesin, DeleteMesin } from "../service/Api";
...
const Mesin = () => {
  ...
  const getData = async () => {
    try {
      const getMesin = await GetAllMesin();
      setMesin(getMesin.data);
    } catch (error) {
  ...
  const handleAdd = () => {
    navigate("/tambah/mesin");
  };
  const handleEdit = (id) => {
    navigate(`/edit/mesin/${id}`);
  };
  ...
  const filteredMesin = mesin.filter((user) =>
    user.name.toLowerCase().includes(search.toLowerCase())
  );

  const totalItems = filteredMesin.length;
  const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const currentMesin = filteredMesin.slice(startIndex, endIndex);
  useEffect(() => {
    setCurrentPage(1);
  }, [search]);
              ...
              <table className="w-full text-left table-auto min-w-max">
                <thead>
                  <tr>
                    ...
                    <th className="p-4 border-b border-slate-500 bg-gray-900">
                      <p className="text-sm font-bold text-grey-200">Kondisi</p>
                    </th>
                    <th className="p-4 border-b border-slate-500 bg-gray-900">
                      <p className="text-sm font-bold text-grey-200">
                        Tanggal Dibuat
                ...
                <tbody>
                  {currentMesin.length > 0 ? (
                    currentMesin.map((user, index) => (
                      <tr
                        key={user.id}
