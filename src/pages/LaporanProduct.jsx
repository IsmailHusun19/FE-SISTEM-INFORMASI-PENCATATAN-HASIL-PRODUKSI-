...
import { GetAllProduct, GetAllShift, GetAllLine, GetLaporanCetakAll} from "../service/Api";
import { useNavigate } from "react-router-dom";
import { useAuth } from "../component/AuthContext";
import { Print } from "@mui/icons-material";

const LaporanProduct = () => {
  ...
  const handleCetakPdf = async () => {
    if (isPrinting) return; // cegah double klik
    ...
    try {
      ...
      const blob = await GetLaporanCetakAll(params);
              ...
              <table className="w-full text-left table-auto min-w-max">
                ...
                <tbody>
                  {currentProduct.length > 0 ? (
                    currentProduct.map((user, index) => (
                      <tr
                      onClick={() => navigate(`/laporan-produksi/${user.id}`)}
                        key={user.id}
