...
const EditLogMesin = () => {
  ...
  useEffect(() => {
    if (!user) return;
    if (user.role !== "OPERATOR_PRODUKSI") navigate("/error");
  }, [user, navigate]);

  if (!user || user.role !== "OPERATOR_PRODUKSI") return null;

  const getData = async () => {
    try {
      const product = await GetAllProduct();
      const produksi = await GetAllProduksi();
      const mesinData = await GetAllMesin();
      setProduk(product.data);
      setMesin(mesinData.data);
      setProduksiAll(produksi.data);
      const log = await GetLogMesinById(id);
      if (log.data) {
        const logData = log.data;
        setForm({
          idProduksi: logData.idProduksi,
          produkId: logData.produksi.produk.id,
          mesinId: logData.mesinId,
          status: logData.status,
          jenisKerusakan: logData.jenisKerusakan || "",
          kerusakan_lainnya: logData.kerusakan_lainnya || "",
          catatan: logData.catatan || "",
        });
        setImages(logData.gambar || []);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    images.forEach((img) => data.append("gambar", img));

    try {
      const res = await EditDataLogMesin(data, id);
      const kondisi = {
        kondisi: form.status === "PARAH" || form.status === "SEDANG",
      };
      await EditDataMesin(kondisi, form.mesinId);

      if (res.status === 200) {
        toast.success("Log mesin berhasil diperbarui", {
          position: "top-right",
        });
        navigate(`/detail/log-mesin/${form.idProduksi}`);
      }
    } catch (err) {
