...
const AddOperatorProduksi = () => {
  ...
  const addData = async (data) => {
    try {
      const res = await AddPengguna(data);
      console.log(res);

      if (res.status === 201) {
        toast.success("Operator Produksi berhasil ditambahkan", {
          position: "top-right",
          description: `${data.name} telah tersimpan`,
        });
        return navigate("/pengguna/operator produksi");
      }

      if (res.status === 400) {
        toast.error("Gagal menambahkan Operator Produksi", {
          position: "top-right",
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Form pendaftaran user dengan role operator produksi
                </p>
              </div>
...
export default AddOperatorProduksi;
