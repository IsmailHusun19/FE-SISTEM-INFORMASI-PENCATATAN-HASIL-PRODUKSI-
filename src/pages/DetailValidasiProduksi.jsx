...
const DetailValidasiProduksi = () => {
  ...
  const handleKonfirmasi = async () => {
    toast.warning("Yakin ingin mengonfirmasi produksi?", {
      description: "Produksi akan dinyatakan selesai",
      position: "top-center",
      duration: Infinity,
      action: {
        label: "Konfirmasi",
        onClick: async () => {
          const loadingToast = toast.loading("Menyimpan konfirmasi...");

          try {
            const res = await EditDataProduksi({ konfrmasi: true }, id);

            if (res.status === 200) {
              toast.success("Produksi berhasil dikonfirmasi", {
                id: loadingToast,
                description: "Produksi telah dinyatakan selesai",
                position: "top-right",
              });
              getData();
            }
          } catch (error) {
            toast.error("Gagal mengonfirmasi produksi", {
              id: loadingToast,
              position: "top-right",
              description:
  ...
  const handleBatalkanKonfirmasi = () => {
    toast.warning("Batalkan konfirmasi produksi?", {
      description: "Status produksi akan dikembalikan seperti semula",
      position: "top-center",
      duration: Infinity,
      action: {
        label: "Batalkan Konfirmasi",
        onClick: async () => {
          const loading = toast.loading("Membatalkan...", {
            position: "top-right",
          });

          try {
            await EditDataProduksi({ konfrmasi: false }, id);
            toast.success("Konfirmasi berhasil dibatalkan", {
              id: loading,
              position: "top-right",
            });
            getData();
  ...
  return (
        ...
        >
          <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <div className="rounded-2xl mt-6 bg-gray-900/80 backdrop-blur border border-gray-800 shadow-xl">
              ...
              <div className="w-full flex pb-5 pr-5 justify-end items-center gap-3">
                {produksi?.konfrmasi ? (
                  <>
                    <span className="text-sm text-green-400 flex items-center gap-1">
                      <CheckCircle size={18} />
                      Produksi telah dikonfirmasi
                      {produksi.unitHead && (
                        <span className="text-slate-400">
                          oleh {produksi.unitHead}
                        </span>
                      )}
                    </span>

                    <button
                      onClick={handleBatalkanKonfirmasi}
                      className="
        h-9 px-3 flex items-center gap-2 text-sm font-semibold
        text-red-300 bg-slate-800
        border border-red-600 rounded
        hover:bg-slate-700 hover:border-red-500
    ...
    "
                  ...
                  /* belum boleh validasi */
                  <span className="text-sm text-yellow-400 italic">
                    Log mesin masih kosong, belum bisa memvalidasi atau
                    mengakhiri produksi ini
                  </span>
                )}
              </div>
            </div>
...
export default DetailValidasiProduksi;
