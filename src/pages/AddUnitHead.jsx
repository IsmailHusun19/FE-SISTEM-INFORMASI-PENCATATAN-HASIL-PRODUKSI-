...
const AddUnitHead = () => {
  ...
  const addData = async (data) => {
    try {
      ...
      if (res.status === 201) {
        toast.success("Unit Head berhasil ditambahkan", {
          position: "top-right",
          description: `${data.name} telah tersimpan`,
        });
        return navigate("/pengguna/unit head");
      }

      if (res.status === 400) {
        toast.error("Gagal menambahkan Unit Head", {
          position: "top-right",
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Form pendaftaran user dengan role Unit Head
                </p>
              </div>
